...
fn main() -> Result<(), TapascoError> {
	...
	device.change_access(tapasco::tlkm::tlkm_access::TlkmAccessExclusive)?;
        ...
        let pe_id = match device.get_pe_id("esa.informatik.tu-darmstadt.de:user:ethernet_example:1.0") {
            Ok(x) => x,
            Err(_e) => continue,
        };
	println!("pe found: {}", pe_id);
        ...
        println!("start");
        pe.start(vec![
            tapasco::device::PEParameter::Single32(10),
            tapasco::device::PEParameter::Single32(322000000),
        ])?;
	let (result, _output) = pe.release(true, true)?;
        println!("done {}", result);
    }
    println!("exit");
    Ok(())
